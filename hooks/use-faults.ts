...
export function useFaultSearch(query: string, filters?: { system?: string; severity?: string; computerRequired?: boolean }) {
  const params = new URLSearchParams()
  if (query) params.append('q', query)
  if (filters?.system) params.append('system', filters.system)
  if (filters?.severity) params.append('severity', filters.severity)
  if (filters?.computerRequired !== undefined) params.append('computerRequired', filters.computerRequired ? 'true' : 'false')
