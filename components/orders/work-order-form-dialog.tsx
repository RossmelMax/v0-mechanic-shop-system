...
export function WorkOrderFormDialog({
    ...
    if (workOrder) {
      form.reset({
        ...
        startDate: workOrder.startDate
          ? new Date(workOrder.startDate).toISOString().split("T")[0]
          : "",
        endDate: workOrder.completionDate
          ? new Date(workOrder.completionDate).toISOString().split("T")[0]
          : "",
  ...
  const availableQuotations = quotations.filter(
    (q) => q.status === "accepted" && !q.workOrder,
  );
